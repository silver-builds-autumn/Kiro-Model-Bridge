...
| `api2kiro.streamDiagnostics` | Default-off duplicate-output diagnostics. Logs each upstream SSE record's type, byte count, SHA-256 digest, conversion counts, and whether a repeated Anthropic text delta was suppressed; it never logs message text or credentials. |
